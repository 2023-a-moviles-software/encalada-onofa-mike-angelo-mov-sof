...
class RegistrarseActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registrarNuevoUsuario.setOnClickListener {
            if(correo.isNotEmpty() && password.isNotEmpty() && nombreUsuario.isNotEmpty()){
                registrarNuevoUsuario(correo.toString(), password.toString(), nombreUsuario.toString())
            }
    ...
    private fun agregarUsuario(userID: String, nombreUsuario: String) {
        ...
        documentoUsuario.set(nuevosDatosUsuario)
            .addOnSuccessListener {
                finish()
            }
            .addOnFailureListener {e ->
                Log.w("Error firestore", "Error al crear la coleccion", e)
            }
    }
